  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-2xl font-bold mb-6">回测数据分析</h1>
        ...
        <div className="flex flex-wrap gap-4 mb-6 items-center">
          ...
          {dateFilter && (
            <button
              className="bg-yellow-500/20 text-yellow-400 px-4 py-2 rounded flex items-center gap-2"
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-zinc-800 p-4 rounded-xl">
            <div className="text-zinc-500 text-sm">总信号</div>
            <div className="text-2xl font-bold">{overallStats.total.toLocaleString()}</div>
          </div>
          <div className="bg-zinc-800 p-4 rounded-xl">
            <div className="text-zinc-500 text-sm">胜率</div>
            <div className={`text-2xl font-bold ${overallStats.winRate >= 55.6 ? 'text-green-400' : 'text-red-400'}`}>
              {overallStats.winRate.toFixed(1)}%
            </div>
          </div>
          <div className="bg-zinc-800 p-4 rounded-xl">
            <div className="text-zinc-500 text-sm">盈利次数</div>
            <div className="text-2xl font-bold text-green-400">{overallStats.wins.toLocaleString()}</div>
          </div>
          <div className="bg-zinc-800 p-4 rounded-xl">
            <div className="text-zinc-500 text-sm">总盈亏</div>
            <div className={`text-2xl font-bold ${overallStats.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        <div className="bg-zinc-800 p-4 rounded-xl mb-6">
          <h2 className="text-lg font-semibold mb-4">按等级统计</h2>
          <div className="grid grid-cols-4 gap-4">
            {levelStats.map(s => (
              <div key={s.level} className="bg-zinc-700/50 p-3 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-zinc-500">{s.total}笔</span>
                </div>
        ...
        {/* 月度盈亏 */}
        <div className="bg-zinc-800 p-4 rounded-xl mb-6">
          <h2 className="text-lg font-semibold mb-4">月度盈亏</h2>
          <div className="overflow-x-auto">
            <div className="flex gap-2 min-w-max">
              {monthlyData.map(m => (
                <div 
                  key={m.month} 
                  className={`bg-zinc-700/50 p-3 rounded-lg w-24 text-center cursor-pointer hover:bg-zinc-600/50 transition-colors ${dateFilter === m.month ? 'ring-2 ring-yellow-400' : ''}`}
                  onClick={() => setDateFilter(dateFilter === m.month ? null : m.month)}
                >
                  <div className="text-xs text-zinc-500 mb-1">{m.month}</div>
                  <div className={`font-bold ${m.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        {/* 每日盈亏 */}
        <div className="bg-zinc-800 p-4 rounded-xl mb-6">
          <h2 className="text-lg font-semibold mb-4">每日盈亏明细</h2>
          {(() => {
            const dailyStats = filteredSignals.reduce((acc, s) => {
            ...
            const maxProfit = Math.max(...dailyData.map(d => d.pnl));
            const maxLoss = Math.min(...dailyData.map(d => d.pnl));
            const avgDaily = dailyData.reduce((sum, d) => sum + d.pnl, 0) / dailyData.length;
              ...
              <>
                <div className="grid grid-cols-5 gap-4 mb-4">
                  <div className="bg-zinc-700/50 p-3 rounded-lg text-center">
                    ...
                    <div className="text-lg font-bold text-green-400">{profitDays}</div>
                  </div>
                  <div className="bg-zinc-700/50 p-3 rounded-lg text-center">
                  ...
                  <div className="bg-zinc-700/50 p-3 rounded-lg text-center">
                    ...
                    <div className={`text-lg font-bold ${avgDaily >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {avgDaily >= 0 ? '+' : ''}{avgDaily.toFixed(1)}U
                    </div>
                  </div>
                  <div className="bg-zinc-700/50 p-3 rounded-lg text-center">
                    <div className="text-xs text-zinc-500">最大单日盈利</div>
                    <div className="text-lg font-bold text-green-400">+{maxProfit.toFixed(0)}U</div>
                  </div>
                  <div className="bg-zinc-700/50 p-3 rounded-lg text-center">
                    <div className="text-xs text-zinc-500">最大单日亏损</div>
                    <div className="text-lg font-bold text-red-400">{maxLoss.toFixed(0)}U</div>
                  </div>
                </div>
        ...
        <div className="bg-zinc-800 p-4 rounded-xl">
          ...
          <div className="overflow-x-auto max-h-96 overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-zinc-800">
                <tr className="text-zinc-500 border-b border-zinc-700">
                  <th className="text-left py-2">入场时间</th>
                  <th className="text-left py-2">结算时间</th>
                  ...
                  <th className="text-right py-2">结算价</th>
                ...
                {(() => {
                  ...
                  return displaySignals.slice().reverse().map((s, i) => (
                    <tr key={i} className="border-b border-zinc-700/50 hover:bg-zinc-700/30">
                      <td className="py-2">{s.timestamp.slice(5, 19)}</td>
                      <td className="py-2">{s.settle_time.slice(5, 19)}</td>
                      ...
                      <td className="py-2 text-right font-mono">{s.entry_price.toLocaleString()}</td>
                      <td className="py-2 text-right font-mono">{s.settle_price.toLocaleString()}</td>
